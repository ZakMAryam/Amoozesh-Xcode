...
struct VocabularyQuestions: Codable {
    let question: String
    var answers: [String]
    let correctAnswer: String
}

struct VocabularyQuizBrian {
    private static let quizProgressKey = "VocabularyQuizProgress"
    
    var questionNumber = 0
    var score = 0
    
    let quiz: [VocabularyQuestions]
    
    init() {
        if let savedProgress = UserDefaults.standard.dictionary(forKey: VocabularyQuizBrian.quizProgressKey),
        ...
        if let url = Bundle.main.url(forResource: "vocabulary", withExtension: "json"),
           ...
           let quizData = try? JSONDecoder().decode([VocabularyQuestions].self, from: data) {
    ...
    private func saveProgress() {
        ...
        UserDefaults.standard.set(progress, forKey: VocabularyQuizBrian.quizProgressKey)
    }
}
